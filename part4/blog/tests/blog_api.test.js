...
describe('test get API', () => {
    ...
    test('blogs are returned has id', async () => {
        const test = await api.get('/api/blogs').set('Authorization', `Bearer ${token}`)
...
describe('test post API', () => {
    test('the blog is posted correctly', async () => {
        ...
        const test = await api.post('/api/blogs').send(data).set('Authorization', `Bearer ${token}`)
        .expect(201)
    ...
    test('the blog is posted without title return 400', async () => {
        ...
        await api.post('/api/blogs').send(data).set('Authorization', `Bearer ${token}`)
        .expect(400)
    }, 100000)
...
describe('test put API', () => {
    ...
    const test = await api.put(`/api/blogs/${blogs[0].id}`)
        .send(data)
